...
/// See also:
///
///  * [CustomRoundedRectangleBorder], which is similar to this class, but with less options to control the appearance of each side/corner.
...
class CustomRoundedRectangleBorder extends ShapeBorder {
  ...
  void paint(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    ...
    if (borderRadius.topLeft.x != 0.0 && paint != null) {
      canvas.drawArc(
        rectForCorner(topLeftCornerSide?.width, rect.topLeft, borderRadius.topLeft, 1, 1),
        pi / 2 * 2,
        pi / 2,
        false,
        paint,
    ...
    if (paint != null) {
      canvas.drawLine(
        rect.topLeft +
            Offset(
              borderRadius.topLeft.x + (borderRadius.topLeft.x == 0 ? (leftSide?.width ?? 0.0) : 0.0),
              (topSide?.width ?? 0.0) / 2,
            ),
        rect.topRight + Offset(-borderRadius.topRight.x, (topSide?.width ?? 0.0) / 2),
        paint,
      );
    }
    ...
    if (borderRadius.topRight.x != 0.0 && paint != null) {
      canvas.drawArc(
        rectForCorner(topRightCornerSide?.width, rect.topRight, borderRadius.topRight, -1, 1),
        pi / 2 * 3,
        pi / 2,
        false,
        paint,
      ...
      canvas.drawLine(
        rect.topRight +
            Offset(
              ...
              borderRadius.topRight.y + (borderRadius.topRight.x == 0 ? (topSide?.width ?? 0.0) : 0.0),
            ),
        rect.bottomRight +
            Offset(
              -1 * (rightSide?.width ?? 0.0) / 2,
    ...
    if (borderRadius.bottomRight.x != 0.0 && paint != null) {
      canvas.drawArc(
        rectForCorner(bottomRightCornerSide?.width, rect.bottomRight, borderRadius.bottomRight, -1, -1),
        pi / 2 * 0,
        pi / 2,
        false,
        paint,
    ...
    if (paint != null) {
      canvas.drawLine(
        rect.bottomRight +
            Offset(
              -borderRadius.bottomRight.x - (borderRadius.bottomRight.x == 0 ? (rightSide?.width ?? 0.0) : 0.0),
    ...
    if (borderRadius.bottomLeft.x != 0.0 && paint != null) {
      canvas.drawArc(
        rectForCorner(bottomLeftCornerSide?.width, rect.bottomLeft, borderRadius.bottomLeft, 1, -1),
        pi / 2 * 1,
        pi / 2,
        false,
        paint,
      ...
      canvas.drawLine(
        rect.bottomLeft +
            Offset(
              ...
              -borderRadius.bottomLeft.y - (borderRadius.bottomLeft.x == 0 ? (bottomSide?.width ?? 0.0) : 0.0),
            ),
        rect.topLeft +
            Offset(
              (leftSide?.width ?? 0.0) / 2,
  ...
  Rect rectForCorner(double? sideWidth, Offset offset, Radius radius, num signX, num signY) {
    sideWidth ??= 0.0;
