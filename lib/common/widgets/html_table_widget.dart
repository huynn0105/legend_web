...
CustomRender customTableRender({OnLinkTapCallback? onLinkTap}) => CustomRender.widget(widget: (context, buildChildren) {
      return CssBoxWidget(
        key: context.key,
        style: context.style,
        child: LayoutBuilder(
          builder: (_, constraints) => _layoutCells(context, constraints, onLinkTap),
...
Widget _layoutCells(
  ...
  for (final row in rows) {
    final cols = row.children.whereType<TableCellElement>().fold(0, (int value, child) => value + child.colspan) +
        rowSpanOffsets.fold<int>(
          0,
          (int offset, child) => child,
        );
...
Widget _renderTableCell(
  ...
  return Container(
    constraints: BoxConstraints(maxWidth: columnMax > 1 ? width / 3 * 2 : width - 35),
